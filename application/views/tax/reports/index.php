        ...
        <form method="GET" class="row g-3">
            <div class="col-md-3">
                ...
                <select name="type" class="form-select" onchange="this.form.submit()">
                    ...
                    <option value="compliance" <?= $report_type === 'compliance' ? 'selected' : '' ?>>Compliance Report</option>
                </select>
            </div>
            <div class="col-md-3">
...
<?php elseif ($report_type === 'compliance'): ?>
    ...
    <div class="card">
        <div class="card-header bg-dark text-white">
            <h5 class="mb-0">Tax Compliance Report</h5>
        </div>
        <div class="card-body">
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <h4 class="mb-0"><?= $compliance_rate ?? 100 ?>%</h4>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-body">
                            <h6 class="text-muted mb-2">Upcoming Deadlines</h6>
                            <h4 class="mb-0"><?= count($upcoming_deadlines ?? []) ?></h4>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <h4 class="mb-0 text-danger"><?= count($overdue_deadlines ?? []) ?></h4>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <?php if (!empty($overdue_deadlines)): ?>
                <div class="alert alert-danger mb-4">
                    <h5><i class="bi bi-exclamation-triangle"></i> Overdue Deadlines</h5>
                    <div class="table-responsive">
                        <table class="table table-sm mb-0">
                            ...
                            <tbody>
                                <?php foreach ($overdue_deadlines as $item): ?>
                                    <tr>
                                        ...
                                        <td><span class="badge bg-danger"><?= floor((time() - strtotime($item['deadline_date'])) / 86400) ?> days</span></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
            ...
            <?php if (!empty($upcoming_deadlines)): ?>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($upcoming_deadlines as $item): ?>
                                <?php $daysRemaining = floor((strtotime($item['deadline_date']) - time()) / 86400); ?>
                                <tr>
                                    <td><?= htmlspecialchars($item['tax_type']) ?></td>
                                    <td><?= htmlspecialchars($item['period_covered'] ?? 'N/A') ?></td>
                                    <td><?= date('M d, Y', strtotime($item['deadline_date'])) ?></td>
                                    <td><?= ucfirst($item['deadline_type']) ?></td>
                                    <td><span class="badge bg-<?= $daysRemaining <= 7 ? 'danger' : ($daysRemaining <= 14 ? 'warning' : 'info') ?>"><?= $daysRemaining ?> days</span></td>
