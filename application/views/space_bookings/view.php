    ...
    <div class="col-md-8">
                ...
                <dl class="row mb-0">
                    ...
                    <dd class="col-sm-8">
                        <a href="<?= base_url('spaces/view/' . $booking['space_id']) ?>">
                            <?= htmlspecialchars($booking['space']['space_name'] ?? 'N/A') ?>
                        </a>
                    </dd>
                    
                    <dt class="col-sm-4">Tenant:</dt>
                    <dd class="col-sm-8">
                        <a href="<?= base_url('tenants/view/' . $booking['tenant_id']) ?>">
                            <?= htmlspecialchars($booking['tenant']['business_name'] ?? $booking['tenant']['contact_person'] ?? 'N/A') ?>
                        </a>
                    </dd>
        ...
        <?php if ($booking['status'] === 'cancelled' && $booking['cancelled_at']): ?>
            <div class="card shadow-sm mt-3">
                ...
                <div class="card-body">
                    <p class="mb-1"><strong>Cancelled:</strong> <?= date('M d, Y g:i A', strtotime($booking['cancelled_at'])) ?></p>
                    <?php if ($booking['cancellation_reason']): ?>
                        <p class="mb-0"><strong>Reason:</strong> <?= nl2br(htmlspecialchars($booking['cancellation_reason'])) ?></p>
                    <?php endif; ?>
                </div>
            </div>
        <?php endif; ?>
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <h4 class="mb-0"><?= format_currency($booking['base_amount']) ?></h4>
                </div>
                
                <?php if ($booking['discount_amount'] > 0): ?>
                    <div class="mb-3">
                        <h6 class="text-muted mb-1">Discount</h6>
                        <h5 class="mb-0 text-success">-<?= format_currency($booking['discount_amount']) ?></h5>
                    </div>
                <?php endif; ?>
                
                <?php if ($booking['tax_amount'] > 0): ?>
                    <div class="mb-3">
                        <h6 class="text-muted mb-1">Tax</h6>
                        <h5 class="mb-0"><?= format_currency($booking['tax_amount']) ?></h5>
                    </div>
                <?php endif; ?>
                ...
                <div class="border-top pt-3">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <h4 class="mb-0 text-primary"><?= format_currency($booking['total_amount']) ?></h4>
                    </div>
                    
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="text-muted">Paid:</span>
                        <strong><?= format_currency($booking['paid_amount']) ?></strong>
                    </div>
                    
                    <div class="d-flex justify-content-between align-items-center">
                        <span class="text-muted">Balance:</span>
                        <strong class="text-<?= $booking['balance_amount'] > 0 ? 'danger' : 'success' ?>">
...
<div class="modal fade" id="cancelModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?= base_url('space-bookings/cancel/' . $booking['id']) ?>" method="POST">
                <?php echo csrf_field(); ?>
